#define _CRT_SECURE_NO_WARNINGS //для небезопасного использования функции sprintf
#include <windows.h>
#include <windowsx.h>
#include <tchar.h>
#include <iostream> //для вывода стандартного звукового сигнала
#include <fstream>
#include <stdio.h>
#include <string>
...
//Поток закраски окна
DWORD WINAPI WindowPaintingThread(LPVOID _hwnd) {
...
HANDLE WPThreadHandle; //дескриптор потока закраски
...
LRESULT CALLBACK MyWndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
	switch (message) {
		...
		case WM_KEYDOWN: {
			...
			//Обработка нажатия Enter - запуск/остановка потока перекраски окна
			if (wParam == VK_RETURN) {
				if (ResumeThread(WPThreadHandle) == 0)
					SuspendThread(WPThreadHandle);

				return 0;
			}
			if (wParam == 0x31) { //клавиша 1
				SetThreadPriority(WPThreadHandle, THREAD_PRIORITY_LOWEST);
				return 0;
			}
			if (wParam == 0x32) { //клавиша 2
				SetThreadPriority(WPThreadHandle, THREAD_PRIORITY_BELOW_NORMAL);
				return 0;
			}
			if (wParam == 0x33) { //клавиша 3
				SetThreadPriority(WPThreadHandle, THREAD_PRIORITY_NORMAL);
				return 0;
			}
			if (wParam == 0x34) { //клавиша 4
				SetThreadPriority(WPThreadHandle, THREAD_PRIORITY_ABOVE_NORMAL);
				return 0;
			}
			if (wParam == 0x35) { //клавиша 5
				SetThreadPriority(WPThreadHandle, THREAD_PRIORITY_HIGHEST);
				return 0;
			}
			if (wParam == 0x36) { //клавиша 6
				SetThreadPriority(WPThreadHandle, THREAD_PRIORITY_TIME_CRITICAL);
				return 0;
			}
		} break;
		case WM_LBUTTONUP: {
...
int main(int argc, char **argv) {
	...
	if (WaitForSingleObject(playersCountSem, 0) == WAIT_TIMEOUT) {
		cout << "\a";
		MessageBox(NULL, "Игра рассчитана на двух игроков. Третья копия будет закрыта.", "Внимание!", 0);
		return 0;
	}
	...
	//Создаём поток перекраски фона
	WPThreadHandle = CreateThread(NULL, 1024, WindowPaintingThread,
		(void *)(hWnd), CREATE_SUSPENDED, NULL); //создаётся спящим
	...
	SuspendThread(WPThreadHandle);
	CloseHandle(WPThreadHandle); //поток окрашивания фона. Завершится сам после завершения главного
